...
# GeoJSON Türkiye il sınırları


df = pd.read_sql("SELECT * FROM siparis_kar", engine)
df["siparis_tarihi"] = pd.to_datetime(df["siparis_tarihi"], errors="coerce")

# PERAKENDE (Toptan) verisini entegre et
toptan_df = pd.read_sql("SELECT * FROM toptan_kar", engine)
...
# -------------------- İADELER --------------------
iade_df = pd.read_sql("SELECT * FROM iadeler", engine)
...
df_filtered["sevk_il"] = df_filtered["sevk_il"].str.strip().str.title()
il_ozet = df_filtered.groupby("sevk_il").agg({
    "satir_fiyat": "sum",
    "kar": "sum"
...
# 📦 Ürün Stok Durumu
# --------------------
urunler_df = pd.read_sql("SELECT * FROM urunler", engine)
